...
write(logObj,'debug','Initializing DynamoDB client');
...
if obj.useCredentialsProviderChain == false
    ...
    % that will not be fixed until AWS SDK version 2 so now an exception is thrown
...
%% check if DynamoDB is available in the given region
availableRegions = RegionUtils.getRegionsForService('dynamodb');
if  ~contains(char(availableRegions), char(awsRegion.getName))
    write(logObj,'error',['Region: ',awsRegion.getName,' does not support DynamoDB']);
end

%% build a handle to the client
builderH = AmazonDynamoDBClientBuilder.standard().withCredentials(awsCredentials).withClientConfiguration(obj.clientConfiguration.Handle);

%% configure the Endpoint if set
if isempty(obj.endpointURI.Host)
    % set the region based on previously determined value
    ...
    write(logObj,'verbose',['Setting Endpoint URI & Region to: ',endpointHostPort,' ',char(endpointRegion)]);
